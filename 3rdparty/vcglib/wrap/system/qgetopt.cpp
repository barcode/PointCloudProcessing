...
QString GetOpt::usage()
{
    ...
    {
        if (options[i].type == Option::OPTION)
            has_options = true;
        if (options[i].type == Option::OPTIONAL)
            has_optionals = true;
        if (options[i].type != Option::ARGUMENT)
            continue;
        u += " <" + options[i].name + ">";
    }
    // optional arguments
    ...
    for (int i = 0; i < options.size(); i++)
    {
        ...
        switch (o.type)
        {
            ...
            case Option::OPTIONAL:
                ...
                break;
            case Option::SWITCH:
                line += "-" + QString(o.o) + " --" + o.name;
                break;
            case Option::OPTION:
                line += "-" + QString(o.o) + " <val> --" + o.name + " <val>";
                break;
            default:
                break;
        }
    ...
    for (int i = 0; i < args.size(); i++)
    {
        ...
        if (arg.startsWith(QString::fromLatin1("--")))
        {
            ...
            {  // OPTION
                ...
                {
                    error = "Error while parsing option " + o.name + ": cannot convert " + arg + " to: " +
                            o.value->typeName();
                    return false;
                }
                *(o.value) = v;
...
bool GetOpt::findOption(char c, Option &option)
{
    ...
    {
        Option &o = options[i];
        if (o.type != Option::OPTION && o.type != Option::SWITCH)
            continue;
        if (o.o == c)
        {
            option = o;
