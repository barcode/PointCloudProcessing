...
#define GL_TEST_ERR                                                                                                    \
    {                                                                                                                  \
        ...
        if ((eCode = glGetError()) != GL_NO_ERROR)                                                                     \
            std::cerr << "OpenGL error : " << gluErrorString(eCode) << " in " << __FILE__ << " : " << __LINE__         \
                      << std::endl;                                                                                    \
    }
...
class SplatRenderer
{
    ...
    void drawSplats(std::vector<std::vector<vcg::Point3f>*>& positions,
                    std::vector<std::vector<vcg::Point3f>*>& normals,
                    std::vector<std::vector<vcg::Point3<unsigned char> >*>& colors, std::vector<float>& radius,
  ...
  public:
    ...
    void Render(std::vector<std::vector<vcg::Point3f>*>& positions, std::vector<std::vector<vcg::Point3f>*>& normals,
                std::vector<std::vector<vcg::Point3<unsigned char> >*>& colors, std::vector<float>& radius,
                vcg::GLW::ColorMode cm, vcg::GLW::TextureMode tm);
...
void SplatRenderer<MeshType>::configureShaders()
{
    ...
    QString shading =
      ...
      " float specularCoeff = aux_dot>0.0 ? clamp(pow(clamp(dot(halfVec, normal),0.0,1.0),gl_FrontMaterial.shininess), "
      "0.0, 1.0) : 0.0;"
      "	return vec4(color.rgb * ( gl_FrontLightProduct[0].ambient.rgb + diffuseCoeff * "
      "gl_FrontLightProduct[0].diffuse.rgb) + specularCoeff * gl_FrontLightProduct[0].specular.rgb, 1.0);"
      "}\n";
    ...
    for (int k = 0; k < 3; ++k)
    {
        ...
        mShaders[k].SetSources(mShaderSrcs[k * 2 + 0] != "" ? vsrc.toLatin1().data() : 0,
...
{
    if ((!mRenderBuffer) || (mRenderBuffer->width() != mCachedVP[2]) || (mRenderBuffer->height() != mCachedVP[3]) ||
        ((mCachedFlags & mRenderBufferMask) != (mFlags & mRenderBufferMask)))
    {
        ...
        mRenderBuffer = new QGLFramebufferObject(mCachedVP[2], mCachedVP[3],
                                                 (mFlags & OUTPUT_DEPTH_BIT) ? QGLFramebufferObject::NoAttachment :
        ...
        if (mFlags & DEFERRED_SHADING_BIT)
        {
            ...
            mRenderBuffer->bind();
            glFramebufferTexture2DEXT(GL_FRAMEBUFFER_EXT, GL_COLOR_ATTACHMENT1_EXT, GL_TEXTURE_RECTANGLE_ARB,
                                      mNormalTextureID, 0);
        ...
        if (mFlags & OUTPUT_DEPTH_BIT)
        {
            ...
            glTexImage2D(GL_TEXTURE_RECTANGLE_ARB, 0, GL_DEPTH_COMPONENT24_ARB, mCachedVP[2], mCachedVP[3], 0,
...
void SplatRenderer<MeshType>::Render(std::vector<MeshType*>& meshes, vcg::GLW::ColorMode cm, vcg::GLW::TextureMode tm)
{
    ...
    mShaders[2].prog.Uniform("viewport", float(mCachedVP[0]), float(mCachedVP[1]), float(mCachedVP[2]),
                             float(mCachedVP[3]));
...
template <class MeshType>
void SplatRenderer<MeshType>::drawSplats(std::vector<MeshType*>& meshes, vcg::GLW::ColorMode cm,
                                         vcg::GLW::TextureMode tm)
{
    ...
    */
    ...
    for (unsigned int ii = 0; ii < meshes.size(); ++ii)
    {
        ...
        glTexCoordPointer(1, GL_FLOAT, size_t(&m.vert[1].cR()) - size_t(&m.vert[0].cR()), &m.vert[0].cR());
