...
class GenericVertexInterpolator
{
  ...
  public:
    inline void operator()(const VertexType &v0, const VertexType &v1, const VertexType &v2, const ScalarType &a,
                           const ScalarType &b, VertexType &r) const
    {
        // position
...
class TriMeshClipper
{
  public:
    ...
    /*
        ...
        Notes:
            ...
            5) vInterp functor must implement a n operator with signature
                   void operator () (const VERTEX & v0, const VERTEX & v1, const VERTEX & v2, const Scalar & a, const
       Scalar & b, VERTEX & r);
    ...
    */
                           ...
                           FACEINDEXCONTAINER &facesToDelete)
    {
            ...
            if (n > 2)
            {
                ...
                for (unsigned int i = 0; i < n; ++i)
                {
                    ...
                    else if (vnfo.fV < eps)
                    {
                        ...
                        std::pair<UIntHMap_i, bool> hi =
                          (*(mi.first)).second.insert(std::make_pair(vmax[1], emptyIsects));
                        bool found = false;
                    ...
                    else if (vnfo.fU < eps)
                    {
                        ...
                        std::pair<UIntHMap_i, bool> hi =
                          (*(mi.first)).second.insert(std::make_pair(vmax[0], emptyIsects));
                        bool found = false;
                        for (unsigned int s = 0; s < (*(hi.first)).second.n; ++s)
                        {
                            if (vClipped[i] == (*(hi.first)).second.isects[s].p)
                    ...
                    else if ((vnfo.fU + vnfo.fV) >= ((ScalarType)(1.0 - 1e-5)))
                    {
                        ...
                        std::pair<UIntHMap_i, bool> hi =
                          (*(mi.first)).second.insert(std::make_pair(vmax[2], emptyIsects));
                        bool found = false;
                        for (unsigned int s = 0; s < (*(hi.first)).second.n; ++s)
                        {
                            if (vClipped[i] == (*(hi.first)).second.isects[s].p)
        ...
        for (size_t i = 0; i < vInfos.size(); ++i)
        {
            if (vInfos[i].idx >= vSize)
            {
                ...
                vInterp(*(m.face[tref].V(0)), *(m.face[tref].V(1)), *(m.face[tref].V(2)), vInfos[i].fV, vInfos[i].fU,
                        m.vert[j]);
                j++;
            }
        }
    ...
    /*
        static inline void Box(const Box3<ScalarType> & b, VERTEXINTEPOLATOR & vInterp, const TriMeshType & m,
       TriMeshType & r);
  ...
  protected:
    ...
    static inline void CrossPoint(int mode, const ScalarType &value, const CoordType &SP, const CoordType &PP,
                                  CoordType &p_out)
    {
        switch (mode)
        {
    ...
    static inline void ClipPolygonLine(int mode, const ScalarType &value, CoordType *P_in, unsigned int n_in,
                                       CoordType *P_out, unsigned int &n_out)
    {
        unsigned int ps;
        CoordType *SP;
        CoordType *PP;
