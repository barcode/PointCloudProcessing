...
{
    inline size_t operator()(const vcg::tri::HashedPoint3i &p) const
    {
        return size_t(p);
    }
};
...
class Clustering
{
  public:
    ...
    // DuplicateFace == bool means that during the clustering doublesided surface (like a thin shell) that would be
    // clustered to a single surface
    ...
    class SimpleTri
    {
      public:
        ...
        {
            return (v[2] != p.v[2]) ? (v[2] < p.v[2]) : (v[1] != p.v[1]) ? (v[1] < p.v[1]) : (v[0] < p.v[0]);
        }
    ...
    // _size is the approximate total number of cells composing the grid surrounding the objects (usually a large
    // number)
