...
{
/*!
* Given a set of elements, it is often useful to break them up or partition them into a number of separate,
* nonoverlapping groups.
* A disjoint-set data structure is a data structure that keeps track of such a partitioning. See
* <a href="http://en.wikipedia.org/wiki/Disjoint-set_data_structure">Diskoint-set data structure on Wikipedia </a> for
* more details.
*/
...
class DisjointSet
{
    ...
    **************************************************/
    ...
    {
        inline size_t operator()(const ObjectPointer &p) const
        {
            return size_t(p);
        }
    };

    std::unordered_map<OBJECT_TYPE *, int, SimpleObjHashFunc> inserted_objects;
    typedef typename std::unordered_map<ObjectPointer, int, SimpleObjHashFunc>::iterator hIterator;
